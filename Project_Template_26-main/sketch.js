...
function handlePlayerArrowCollision() {
  var collision=Matter.SAT.collides(playerArrows[i].body,computer[j].body)
  if(collision.collided){
    computer[j].remove(j)
    Matter.World.remove(world,playerArrows[i].body)
    playerArrow.splice(i,1);
    i--
  }
}

function handleComputerArrowCollision() {
  var collision=Matter.SAT.collides(computerArrows[l].body,player[k].body)
  if(collision.collided){
    player[k].remove(k)
    Matter.World.remove(world,computerArrows[i].body)
    computerArrow.splice(l,1);
    l--
  }
